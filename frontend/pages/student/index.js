...
export default function TeacherPage({ data }) {
    ...
    return (
        <div className="flex flex-col w-full min-h-screen bg-art md:flex-row">
            {/* Toggle Side-Menu Button*/}
            <button
                onClick={toggleMenu}
                className='sticky top-0 z-50 h-screen p-4 text-white bg-slate-900/40 backdrop-blur-lg'
            >
            ...
            {/* Dashboard */}
            <div className="flex flex-col items-center flex-1 px-20 py-6 overflow-y-scroll border-r-2 border-slate-900">
                <HeadingXL text={"Student Dashboard"} />
                {/* Materials Section */}
                <section className="my-10">
                    <HeadingL text={"Levels"} />
                    <ul className="flex flex-row flex-wrap items-center justify-center gap-5">
                        {Object.keys(data).map((item, index) => (
                            <li key={index}>
                                <Link
                                    href={"/student/levels/" + item}
