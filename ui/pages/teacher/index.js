...
export default function TeacherPage() {
    ...
    const url2 = 'http://127.0.0.1:5000/list-user-conversations/' + getCookie('email')
    ...
    return (
        <div className="min-h-screen w-full bg-art flex flex-row">
            ...
            {/* Dashboard */}
            <div
                className="flex-1 flex flex-col items-center overflow-y-scroll py-10"
            >
        ...
        <section className="my-10 w-10/12">
            ...
            <ul className="flex flex-row flex-wrap items-center justify-center gap-5">
                ...
                <li className="conversation-card">
                    <Link href={"/teacher/create"} className="flex h-full w-full items-center justify-center rounded-2xl bg-slate-900/40 text-center backdrop-blur">
                ...
                {props.data && props.data.map((item, index) =>
                    <li key={index}>
                        <Link href={"/student/conversation/" + item['_id']} className="conversation-card">
        ...
        <section className="my-10">
            ...
            <ul className="flex flex-row flex-wrap items-center justify-center gap-5">
                ...
                <li className="student-card bg-sky-900">
                    <Link href={"/teacher/manage-students"} className="h-full w-full flex justify-center items-center">
                        <FiUserPlus size={36} />
                    </Link>
                </li>
                {/* List of Students */}
                {props.students.map((student, index) => <li key={index} className="student-card font-display text-md">
                    {student}
                </li>)}
            </ul>
        </section>
