...
export default function ManageStudentsPage() {
  ...
  // Fetch All Students
  const url1 = `${BACKEND_URL_BASE}/get-all-students`;
  ...
  const url2 = `${BACKEND_URL_BASE}/get-friends/${getCookie("email")}`;
  ...
  const add = async (data) => {
    const url = `${BACKEND_URL_BASE}/create-friendship`;
    try {
  ...
  const remove = async (data) => {
    const url = `${BACKEND_URL_BASE}/remove-friendship`;
    try {
      const response = await fetch(url, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    // MAIN
    <div className="min-h-screen bg-art flex flex-col items-center justify-center">
      <Head>
        <title>TeachMe | Manage Students</title>
      </Head>
      {/* Container */}
      <div className="flex flex-col items-center gap-8 bg-slate-900/40 backdrop-blur-md shadow-md p-10 w-full h-full">
        {/* Heading */}
        <HeadingXL text={"Manage Students"} />
        {/* Error */}
        {isError && (
          <h1 className="text-rose-900 font-bold text-2xl">Server Error!</h1>
        )}
        {/* Add Student Section */}
